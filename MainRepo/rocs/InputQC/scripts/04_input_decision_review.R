...
cat("The current network now uses sample-wise CLR on `tax_abund_tad`, so each sample is centered against the taxa observed within that sample.\n")
cat("This keeps the main pipeline aligned with the compositional framing we previously treated as a stress test.\n\n")
cat("Why this still matters: sequencing-depth and preservation structure can remain biologically entangled even after sample-wise centering, so we still compare nearby alternatives rather than assuming the new baseline removes every technical axis.\n\n")
cat("This review asks whether the current `exp3` modules still tell the same biological story when we compare the promoted input frame against the remaining alternatives.\n\n")
...
cat("## Outputs\n\n")
cat("- Summary: `InputQC/results/decision/input_decision_summary.tsv`\n")
cat("- Module matching: `InputQC/results/decision/module_match_table.tsv`\n")
cat("- Technical correlations: `InputQC/results/decision/module_technical_correlations.tsv`\n")
cat("- Biological correlations: `InputQC/results/decision/module_biological_correlations.tsv`\n")
cat("- Heatmap: `InputQC/results/figures/input_decision_heatmap.png`\n")
sink()
