...
# if (length(args) < 1) {
#   stop("Usage: Rscript butteracid_rerun.R CONFIG_FILE", call. = FALSE)
# }
# config_file <- args[1]
...
# #####################################################
# # Raw LCA: combined file from all {sample}.lca.gz files
# #####################################################
# raw_lca_all_file <- file.path(OUTPUT_DIR, "raw_lca_all.tsv")

# raw_lca_all_df <- cache_read(raw_lca_all_file, fread, sep = "\t", data.table = FALSE)

# if (is.null(raw_lca_all_df)) {
#   log_msg("raw_lca_all cache missing; reading per-library .lca.gz files ...")
#   raw_lca_all_df <- read_or_process(
#     raw_lca_all_file,
#     EUK_DIR,
#     # matches: {sample}.lca.gz
#     "\\.lca\\.gz$"
#   )
#   raw_lca_all_df <- cache_write(raw_lca_all_df, raw_lca_all_file, fwrite, sep = "\t", quote = FALSE)
#   log_msg(paste("Raw LCA rows:", nrow(raw_lca_all_df)))
# } else {
#   log_msg(paste("Loaded raw LCA rows:", nrow(raw_lca_all_df)))
# }
