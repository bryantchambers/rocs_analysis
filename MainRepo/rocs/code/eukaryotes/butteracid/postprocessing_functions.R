...
dmg_fwd_CCC <- function(df, smp, ci = "z-transform", nperm = NULL, nproc = 1) {
  ...
  if (nproc > 1 && length(chunks) > 1) {
    old_plan <- future::plan()
    on.exit(future::plan(old_plan), add = TRUE)
    options(future.globals.maxSize = 2291289600)
    future::plan(future::multisession, workers = nproc)
    results <- furrr::future_map_dfr(chunks, ccc_worker, .progress = TRUE)
  } else {
    results <- purrr::map_dfr(chunks, ccc_worker)
  }
...
summarise_stats_joined_all <- function(
  ...
  dplyr::as_tibble(stats_joined) |>
    ...
    dplyr::summarise(
      ...
      dplyr::across(dplyr::all_of(mean_cols), ~ median(.x, na.rm = TRUE)),
...
summarise_bf_genus <- function(
  ...
  bf_md_data %>%
    ...
    summarise(
      ...
      across(all_of(mean_cols), ~ median(.x, na.rm = TRUE)),
